...
public class SchemaRepositoryImpl implements SchemaRepository {
    ...
    private String buildColumnDefinition(com.hussain.definition_api.core.model.ColumnDefinition col) {
        ...
        // Add IDENTITY if specified - FIXED
        if (col.isIdentity()) {
            def.append(" GENERATED BY DEFAULT AS IDENTITY");

            if (col.getIdentityStartWith() != null || col.getIdentityIncrementBy() != null) {
                def.append(" (");
                boolean hasStart = false;

                if (col.getIdentityStartWith() != null) {
                    def.append("START WITH ").append(col.getIdentityStartWith());
                    hasStart = true;
                }
        ...
        if (col.getDefaultValue() != null) {
            ...
            if (isPostgreSQLFunction(defaultValue)) {
                ...
                if (defaultValue.equalsIgnoreCase("true") || defaultValue.equalsIgnoreCase("false")) {
                    def.append(defaultValue.toLowerCase()); // No quotes for boolean
                } else {
    ...
    private boolean isPostgreSQLFunction(String value) {
        ...
        for (String func : functions) {
            if (upper.equals(func) || upper.startsWith(func + "(") || upper.startsWith(func + " ")) {
                return true;
            }
        }
        ...
        if (upper.matches("^[A-Z_]+\\(")) {  // Remove the extra ) from the regex
            return true;
        }
